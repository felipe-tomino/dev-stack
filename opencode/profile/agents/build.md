...
Load `work-spec` for every implementation and persist a concise session-local record with `work_spec_write` before editing. `work_spec_write` is a model-facing OpenCode tool: call it directly with the complete record. Never look for it with Bash, run `command -v`, or substitute a file. Loading the skill is not persistence: do not make the first edit until the direct `work_spec_write` call succeeds, even for a trivial change; if that call is unavailable or fails, stop without editing. Update the record when decisions, slices, blockers, verification, or the resume point change. Call the model-facing `work_spec_read` tool directly to recover it after compaction or handoff. Scale the detail to the work. Do not create a repository artifact unless the user requests one, and do not ask the user to approve wording separately for clear low-risk work.
